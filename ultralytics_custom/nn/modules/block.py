...
class PConv(nn.Module):
    ...
    入力チャンネルの一部だけに 3x3 Conv をかけ、残りはスキップして結合することで FLOPs を削減します。
    """
    def __init__(self, c: int, k: int = 3, s: int = 1, ratio: float = 0.25, act: bool = True):
        """Initialize Partial Convolution layer.
        Args:
            c (int): Number of input channels.
            k (int): Kernel size.
            s (int): Stride.
            ratio (float): Ratio of channels to apply convolution.
            act (bool): Whether to apply activation function.
        """
        super().__init__()
        self.c = c
        self.ratio = ratio
        c_p = max(1, int(c * ratio)) # 畳み込みをかけるチャンネル数
        self.c_p = c_p
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            torch.Tensor: Output tensor after applying Partial Convolution.
        """
        c_p = self.c_p
        x_p, x_skip = x[:, :c_p], x[:, c_p:]  # 分割 + スキップ接続
        y = self.act(self.bn(self.conv(x_p)))  # 畳み込み、バッチ正規化、活性化
        # チャンネル数をCに合わせる
        return torch.cat((y,x_skip), 1)  # 結合

class PWConv(nn.Module):
    """
    Point-wise (1x1) Convolutionの薄いラッパー
    """
...
class FasterBlock(nn.Module):
    ...
    PConv → PWConv(拡張) → PWConv(圧縮) に、入力との残差を足します。
    """
    def __init__(self, c: int, expansion: float = 2.0, ratio: float = 0.25):
        """Initialize FasterBlock.
        Args:
            c (int): Number of input/output channels.
            expansion (float): Expansion ratio for the middle layer.
            ratio (float): Ratio of channels to apply Partial Convolution.
        """
        super().__init__()
        c_mid = int(c * expansion)
        self.pconv = PConv(c, k=3, s=1, ratio=ratio, act=True)
        self.pw1 = PWConv(c, c_mid, act=True)  # 拡張
        self.pw2 = PWConv(c_mid, c, act=False)  # 圧縮

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Apply FasterBlock to input tensor.
        Args:
            x (torch.Tensor): Input tensor.
        Returns:
            torch.Tensor: Output tensor after applying FasterBlock.
        """
        y = self.pconv(x)
        y = self.pw1(y)
        y = self.pw2(y)
...
class FastC2f(nn.Module):
    ...
    Ultralytics の C2f をベースに、内部の Bottleneck を FasterBlock に置き換えたもの。
    """
    def __init__(
        self, 
        c1: int, 
        c2: int, 
        n: int = 1, 
        shortcut: bool = False, #互換性のためだけの引数 
        g: int = 1, #同上
        e: float = 0.5,
        ratio: float = 0.25 # PConv のチャネル比率
    ):
        """Initialize FastC2f module.
        Args:
            c1 (int): Input channels.
            c2 (int): Output channels.
            n (int): Number of FasterBlock modules.
            shortcut (bool): Unused, for compatibility only.
            g (int): Unused, for compatibility only.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Forward pass through FastC2f module.
        Args:
            x (torch.Tensor): Input tensor.
        Returns:
            torch.Tensor: Output tensor after applying FastC2f.
        """
